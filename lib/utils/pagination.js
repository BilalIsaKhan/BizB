...
export const loadNextPage =
  ({ queryName, data, limit, fetchMore, routingStore }) =>
  () => {
    ...
    fetchMore({
      ...
      updateQuery: (previousResult, { fetchMoreResult }) => {
        ...
        if (!fetchMoreResult) return previousResult; // No new data, return previous result

        const { [queryName]: previousItems } = previousResult;
        const { [queryName]: newItems } = fetchMoreResult;

        const mergedItems = {
          ...previousItems,
          edges: [...previousItems.edges, ...newItems.edges],
        };

        console.log("merged items", mergedItems);

        mergedItems.pageInfo = newItems.pageInfo;

        // Return the merged result
        return {
          [queryName]: mergedItems,
        };
      },
    });
  };
